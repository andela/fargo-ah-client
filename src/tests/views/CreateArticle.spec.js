...
describe('Testing the create article component', () => {
  ...
  const rejectPromiseSpy = sinon.spy(() => Promise.reject(response));
  const wrapper = shallow(<Article getTags={spy} creatNewArticle={resolvePromiseSpy} />);
  const wrapper1 = shallow(<Article getTags={spy} creatNewArticle={rejectPromiseSpy} />);
  ...
  it('Should call the submit method on submit', () => {
    ...
    expect(rejectPromiseSpy).toHaveProperty('callCount', 1);
  });
