...
import api, { setAuthorizationHeader } from '../../redux/api';
...
describe('Testing API calls', () => {
  ...
  it('should dispatch SET_CURRENT_USER ACTION after editing profile', (done) => {
    moxios.wait(() => {
      const request = moxios.requests.mostRecent();
      request.respondWith({
        status: 200,
    ...
    const store = mockStore({ currentUser: {} }, expectedActions, done);
    setAuthorizationHeader('token not real the real one');
    store.dispatch(editAction(userData)).then(() => {
      const actions = store.getActions();
      expect(actions[0].type).toEqual(actionTypes.SET_CURRENT_USER);
      done();
    });
